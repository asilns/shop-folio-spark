...
import { storeFrom, storeInsert, validateStoreId } from '@/lib/storeScope';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (showNewCustomerForm) {
        ...
        const { data: customerData, error: customerError } = await storeInsert('customers', storeId, {
      ...
      const { data: orderData, error: orderError } = await storeInsert('orders', storeId, {
      ...
      const { error: itemsError } = await (supabase as any)
        .from('order_items')
        .insert(orderItemsData.map(item => ({ ...item, store_id: storeId })));
