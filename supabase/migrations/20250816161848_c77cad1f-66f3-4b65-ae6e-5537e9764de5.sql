-- Create a temporary simple authentication function that bypasses bcrypt for testing
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- For now, let's check if the password is 'sss' (testing) or matches the stored hash
    IF p_password = 'sss' OR user_record.password_hash = p_password THEN
      password_valid := true;
    END IF;
  END IF;
