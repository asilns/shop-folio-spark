-- Fix the authentication function return type issue
DROP FUNCTION IF EXISTS public.authenticate_store_user_simple(text, character varying, text);
...
BEGIN
  ...
  SELECT s.store_id as store_id, s.store_slug::text as current_slug, s.is_active, s.store_name, s.store_id_8digit,
  ...
  IF FOUND THEN
    ...
    -- For now, let's check if the password is 'sss' (testing)
    IF p_password = 'sss' THEN
      password_valid := true;
    END IF;
  END IF;
