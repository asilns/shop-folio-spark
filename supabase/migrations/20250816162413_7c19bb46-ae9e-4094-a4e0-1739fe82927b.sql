-- Remove the UPDATE to test authentication in read-only mode
...
BEGIN
  ...
  -- Resolve store by slug or name - use correct column references
  ...
  -- Find user by username within this store - use correct store_id reference
  ...
  IF FOUND AND password_valid THEN
    -- Return user data (removed UPDATE for read-only testing)
    RETURN QUERY
    SELECT 
